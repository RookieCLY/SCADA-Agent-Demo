...
class JudgeResult(BaseModel):
	...
	def _normalize_failure_category(self) -> JudgeResult:
		# Only backfill "other" when a trace failed without a category.
		# Relaxed guardrails (final_state_match / trajectory_match caps at 0.95) can
		# produce passed=True traces that still carry a failure_category; keep it.
...
def apply_deterministic_guardrails(
	...
	*,
	...
	"""Cap LLM judge scores when deterministic checks prove a failure.

	The LLM judge is useful for semantic nuance, but deterministic metrics are
	authoritative for final-state, expected-error, and trajectory gates. This
	post-processing keeps a permissive judge response from turning a known
	failed trace into a perfect score.
	"""
	if not metrics:
		return result
	...
	if row.get("final_state_match") is False:
		overall_cap = min(overall_cap, 0.95)
		task_cap = min(task_cap, 0.6)
		failure_category = failure_category or "missing_final_state"
		reasons.append("final_state_match=false")

	if row.get("expected_error_code_match") is False:
		overall_cap = min(overall_cap, 0.3)
		task_cap = min(task_cap, 0.3)
		failure_category = failure_category or "expected_error_mismatch"
		reasons.append("expected_error_code_match=false")

	if row.get("trajectory_match") is False:
		overall_cap = min(overall_cap, 0.95)
		tool_cap = min(tool_cap, 0.6)
		step_cap = min(step_cap, 0.6)
		failure_category = failure_category or "trajectory_violation"
		reasons.append("trajectory_match=false")

	if row.get("hallucinated") or row.get("out_of_scope") or row.get("forbidden_tools_violated"):
		overall_cap = min(overall_cap, 0.4)
		tool_cap = 0.0
		failure_category = failure_category or "wrong_tool"
		reasons.append("invalid or forbidden tool call")

	if row.get("parameter_validity") is not None:
		param_cap = min(param_cap, _score(row.get("parameter_validity"), default=1.0))
	if row.get("parameter_match") is not None:
		param_cap = min(param_cap, _score(row.get("parameter_match"), default=1.0))
	if param_cap < 1.0:
		overall_cap = min(overall_cap, max(0.5, 0.79 * param_cap))
		failure_category = failure_category or "wrong_parameters"
		reasons.append("parameter check below perfect")

	if row.get("loop_stuck"):
		overall_cap = 0.0
		task_cap = 0.0
		step_cap = 0.0
		failure_category = failure_category or "inefficient_or_looping"
		reasons.append("loop_stuck=true")
		hard_gate = True

	if not reasons:
		return result

	overall = min(result.overall, overall_cap)
	task_completion = min(result.task_completion, task_cap)
	tool_correctness = min(result.tool_correctness, tool_cap)
	parameter_correctness = min(result.parameter_correctness, param_cap)
	step_efficiency = min(result.step_efficiency, step_cap)
	passed = bool(overall >= pass_threshold and not hard_gate)
	reason_suffix = " Deterministic guardrails applied: " + "; ".join(reasons) + "."
	...
	return JudgeResult(
		...
		clarification_or_rejection_quality=result.clarification_or_rejection_quality,
...
def judge_trace(
	...
	*,
	...
	return apply_deterministic_guardrails(result, metrics)
