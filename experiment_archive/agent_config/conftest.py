...
def _auto_upgrade_build_llm(request, monkeypatch):
    ...
    * Real LLM configs are downgraded to ``MockLLM`` if their corresponding
      key is missing, so configs like ``configs/xiaomi_mimo_smoke.yaml`` or
      ``configs/glm_smoke.yaml`` no longer crash test_configs.py when running
      on a fresh checkout.
    ...
    def patched(cfg, *, registry=None, arch=None):
        llm_mod._load_dotenv_into_environ()
        has_key = False
        if cfg.provider == "mock":
            has_key = True
        elif cfg.provider == "xiaomi-mimo":
            has_key = bool(llm_mod._env("XIAOMI-MIMO_API_KEY", "XIAOMI_MIMO_API_KEY"))
        elif cfg.provider == "deepseek":
            has_key = bool(llm_mod._env("DEEPSEEK_API_KEY"))
        elif cfg.provider == "openrouter":
            has_key = bool(llm_mod._env("OPENROUTER_API_KEY"))
        elif cfg.provider == "glm":
            has_key = bool(llm_mod._env("GLM_API_KEY"))
