...
# ── helpers ────────────────────────────────────────────────────────────────────
def _build_run_id(prefix: str, top_k: int, abbr: str, provider: str) -> str:
    clean_provider = provider.replace("-", "_").replace("/", "_")
    return f"{prefix}_k{top_k}_{abbr}_{clean_provider}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    for provider in providers:
        ...
        for k in top_k_values:
            run_id = _build_run_id(args.prefix, k, abbr, provider)
            w = pd.get("workers", 3)
