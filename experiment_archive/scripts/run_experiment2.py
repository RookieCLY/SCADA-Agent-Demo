...
# ── helpers ────────────────────────────────────────────────────────────────────
def _build_run_id(prefix: str, tool_count: int, abbr: str, provider: str) -> str:
    clean_provider = provider.replace("-", "_").replace("/", "_")
    return f"{prefix}_tc{tool_count}_{abbr}_{clean_provider}"
    ...
    for provider in providers:
        ...
        for tc in tool_counts:
            for cfg_path, abbr, desc in SWEEP_CONFIGS:
                run_id = _build_run_id(args.prefix, tc, abbr, provider)
                w = pd.get("workers", 3)
