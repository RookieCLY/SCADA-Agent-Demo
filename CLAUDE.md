...
7. **Multi-Agent collaboration** (`architecture.multi_agent.enabled`, `agent/multi_agent.py`) — the *agent-loop* lever, orthogonal to levers 1–4 (those gate the tool **surface**; this changes **who** holds the conversation). A–H are a single agent carrying the whole task in one growing context over a cross-domain tool ranking. With this on, a deterministic **Supervisor** routes the query to per-state **Specialists** from the existing Tool-RAG ranking (no extra LLM call), each Specialist runs a bounded *private* conversation (`turns_per_specialist`) over only its own state's tools (`tools_per_specialist`), a **Blackboard** hands forward the entity IDs actually created (read from `world_diff`, not model narration), and a deterministic **Critic** re-runs a Specialist exactly once when its slice produced no world change. No cage is weakened: a Specialist is narrower than the single agent, every dispatch passes the assignment check plus the §4.7 policy, the Supervisor enters states via **legal** FSM transitions only, and Specialist turns spend the same global `max_turns` budget. Decomposition, retries and the board land in the trace under `crew`. `configs/I_multi_agent.yaml` = F + this.
