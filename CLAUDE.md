...
7. **Plan-and-Execute** (`architecture.plan_execute.enabled`, `agent/planner.py`) — the *agent-loop* lever, orthogonal to levers 1–4 (those gate the tool **surface**; this changes **when** the model is asked). A–H run an interleaved loop: one LLM call per tool call, nothing checked before dispatch, ordering decided one step at a time. With this on, `Agent.run` asks the backend's `make_plan` hook for the whole ordered sequence (one call), **compiles** it deterministically in `compile_plan` — drop hallucinated tools, validate every argument object against its Pydantic schema, collapse duplicates, topologically repair the order from the `intended_entities`/`referenced_entities` contract — then executes with no LLM in the loop, replanning only on a real step failure (`max_replans`). Cages are untouched: `state_route` walks **legal** FSM transitions only (never `force_to`), the §4.7 policy is checked before every dispatch, and a `POLICY_DENIED` step ends the run instead of triggering a replan. Backends without `make_plan` (e.g. `MockLLM`) abstain and the run falls back to the interleaved loop, which is what keeps the mock-scripted tests deterministic. Plan size and every compiler rejection land in the trace under `plan`. `configs/I_plan_execute.yaml` = F + this.
