...
- **All 106 cases declare an `expected_trajectory`.** They used to be 12, which meant `trajectory_match` / `forbidden_tools_violated` / `step_efficiency` / `strict_success` were computed over ~11% of the dataset and reported as if they covered it — at 12 cases one case is ~8pp, so no few-point trajectory or safety claim was supportable. `scripts/annotate_golden_trajectories.py` owns the annotation table and regenerates both the JSONL and `eval/golden_cases/`; `--check` asserts they are in sync (also covered by `tests/test_golden_schema.py`). Edit the table, not the JSONL. Three conventions the metrics rely on: `|`-alternation in `required_tools`/`required_actions` (either spelling satisfies the step), `forbidden_tools` matching domains *and* atomics *and* actions (so a ban bites in flat and hierarchical mode alike), and `allowed_terminal_states` with `!STATE` exclusions — a literal `DONE` would score whether the model emitted `next_state: DONE`, which is prompt compliance and config-correlated, not task completion. `eval/golden_dataset.v1.jsonl` is the pre-widening snapshot for re-scoring archived runs under their original expectations.
