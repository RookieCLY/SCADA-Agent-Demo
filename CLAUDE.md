...
8. **Plan-tier closed-loop levers** (`architecture.plan_execute`). Each targeted one measured class of the residual gap between the flat baseline A and the shipping J — A wins 25 of 212 runs, J wins 18, and the 25 split by *mechanism*, not by difficulty. **Measured verdict (`results_w10`/`results_w11`, 106 × 3): only the first is on.** `clarify_on_underspecified` is promoted into `J_combined.yaml`; the cascade guard and the verify round are **off on evidence** and their configs survive only as recorded negatives. Note the planner-prompt changes below are *unconditional*, so no lever setting reproduces the archived A–J prompt:
   ...
   **`results_w28` — the K14 wave, and why the shipping number did not move.** Post-w25 residue mining found six candidate mechanisms; the wave was validated in three iterations against archived w25-J (82.88, 5 reps), each J×3 on the same seeds/relay, with a guard tripwire at case 40 and a hard rule: *no ship without statistical significance, no ship with guard regressions.*

   - **Reverted on measurement, twice.** v2 added a delegation-clarify rule + role-naming vocabulary + auto-template prefix to the planner prompt: −1.47pp, ledger −8. The instructive failure: this dataset expects "全套搞定" (golden-026) to *act* but "你都看着办" (golden-090) to *ask* — no prompt rule can sit on that line, and the tripwire caught the golden-040 regression at case 40 in the first attempt (auto-prefix leaked into a delegation case; a rep with a mid-run prompt change was restarted from zero, twice). v3 dropped the prompt, kept a reordered bind_point vocabulary (`text→value|text`): still −1.47 — the reorder consistently broke golden-029's label *rebind* (−0.60 in both waves) while its target (golden-088) converted in only one wave. Reverted to the exact w25 wording.
   - **Kept on correctness, claimed as neutral.** v4 = the five pure repairs only: `_page_diff` field-granularity (page-property setters no longer claim every widget — golden-032's "preserve the widgets" was unsatisfiable *via the correct tools*), `clone_page` re-parents cloned widgets (they kept the source's `page_id`), `batch_create_points` gains `start` + separator-aware tags ("PT-100到PT-110" was previously inexpressible; the only possible call made `PT_1..11` and every downstream alarm died), the HH/LL→`priority='high'` convention note, and colour-notation equivalence in the metric (`red` ≡ `#FF0000`; golden-007 wanted `#FFFFFF` while golden-087 wanted `red` *verbatim* — a notation lottery; archived arms rescore identically, so it is a fairness repair, not a score gift). Verdict: **+1.41pp, CI [−0.38, +3.40], p = 0.17, guards net 0** — kept because reverting would restore known defects, but **no accuracy claim is made**; the headline stays w25's J = 82.88 / +22.50 over A, measured before these repairs.
   - **The honest reading of the remaining residue:** after K14, J's failures are dominated by dataset naming-lottery (expected ids like `Temp`/`chem`/`backup`/`#001A33` that no convention derives), the relay's intermittent function-calling flakiness hitting the fallback tier (golden-103's failing reps are all `loop=interleaved`), and single-rep terra nondeterminism. The mechanism well is, for now, dry.
