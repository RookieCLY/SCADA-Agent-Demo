...
7. **ReAct turn structure** (`architecture.react.enabled`, `agent/react.py`) — the *agent-loop* lever, orthogonal to levers 1–4 (those gate the tool **surface**; this changes how a turn is **conducted**). A–H run an act-only loop: the model's reasoning is discarded between turns and every raw tool payload is threaded back verbatim, so `input_tokens` grows with the square of the tool-call count and the model re-derives intent from a bare error code. ReAct persists a bounded Thought → Action → Observation scratchpad (`scratchpad_window` triples) rendered into the prompt, **compresses** tool payloads into observations before threading them back, annotates failures with an error-code-keyed repair hint, and answers a repeated identical action from the scratchpad instead of dispatching it again. The dedupe rule runs *after* the state-machine whitelist and the §4.7 policy check, so it can never turn a blocked call into an executed one. Per-run stats land in the trace under `react`. `configs/I_react.yaml` = F + this.
